...
def letterFreqs(lets, is_con):
    ...
    :param is_con: bool is the letter a consonant
...
def words(lnth=random.randrange(5, 15)):
    """
...
def addPhoneme(word):
    ...
    if is_onset:
        if cons_number < 2 and (last_son <= 1 or last_son == 9):
            cons_or_vowel = random.randrange(0, 4)
            if cons_or_vowel == 1 or cons_number == 0:
                for i in Base.cons:
                    if (last_son < i[2]) or last_son == 9:
        ...
        if cons_number < 1:
            cons_or_vowel = random.randrange(0, 6)
            if cons_or_vowel == 1:
                for i in Base.cons:
                    possible_phons.append(i)
                l_freqs = letterFreqs(possible_phons, True)
